...
// connect to postgis
$conn = connect_pg("worldgisdb");
if (!$conn) {
	die ("Connnection to database failed.");
}

$Gazstring = "Kyoto";
$sql = 'SELECT name, Astext(the_geom) FROM gaz';
$res = pg_query($conn,$sql) or die("Failed to retrieve the data!");
$rownum = pg_numrows(res$);
for($i=0; $i<$rownum; $i++){
  $arr = pg_fetch_row($res,$i);
  if($arr[0] == $Gazstring){
    $arrexp = explode(" ", $arr[1]);
    $arrexp[0] = substr($arrexp[0],6);
    $arrexp[1] = substr($arrexp[1],0,strlen($arrexp[1])-1);
  }
}
pg_close($conn);
